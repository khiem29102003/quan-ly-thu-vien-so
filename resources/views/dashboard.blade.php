...
@section('title', 'Dashboard - Quản Lý Thư Viện')
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 0.75rem; margin-bottom: 1.5rem;">
    <div class="stat-card-premium glass-effect" style="background: linear-gradient(135deg, rgba(6, 182, 212, 0.95) 0%, rgba(8, 145, 178, 0.95) 100%); border: 1px solid rgba(255,255,255,0.2);">
        <div style="display: flex; flex-direction: column; align-items: center; text-align: center; gap: 0.5rem;">
            ...
            <h3 style="font-size: 1.5rem; margin: 0; color: white; font-weight: 700;">{{ $stats['total_books'] }}</h3>
    ...
    <div class="stat-card-premium glass-effect" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.95) 0%, rgba(5, 150, 105, 0.95) 100%); border: 1px solid rgba(255,255,255,0.2);">
        <div style="display: flex; flex-direction: column; align-items: center; text-align: center; gap: 0.5rem;">
            <div style="font-size: 2rem;">✅</div>
            <h3 style="font-size: 1.5rem; margin: 0; color: white; font-weight: 700;">{{ $stats['available_books'] }}</h3>
    ...
    <div class="stat-card-premium glass-effect" style="background: linear-gradient(135deg, rgba(139, 92, 246, 0.95) 0%, rgba(124, 58, 237, 0.95) 100%); border: 1px solid rgba(255,255,255,0.2);">
        <div style="display: flex; flex-direction: column; align-items: center; text-align: center; gap: 0.5rem;">
            <div style="font-size: 2rem;">👥</div>
            <h3 style="font-size: 1.5rem; margin: 0; color: white; font-weight: 700;">{{ $stats['total_users'] }}</h3>
    ...
    <div class="stat-card-premium glass-effect" style="background: linear-gradient(135deg, rgba(239, 68, 68, 0.95) 0%, rgba(220, 38, 38, 0.95) 100%); border: 1px solid rgba(255,255,255,0.2);">
        <div style="display: flex; flex-direction: column; align-items: center; text-align: center; gap: 0.5rem;">
            ...
            <h3 style="font-size: 1.5rem; margin: 0; color: white; font-weight: 700;">{{ $stats['overdue_borrows'] }}</h3>
    ...
    <div class="stat-card-premium glass-effect" style="background: linear-gradient(135deg, rgba(14, 165, 233, 0.95) 0%, rgba(2, 132, 199, 0.95) 100%); border: 1px solid rgba(255,255,255,0.2);">
        <div style="display: flex; flex-direction: column; align-items: center; text-align: center; gap: 0.5rem;">
            <div style="font-size: 2rem;">📊</div>
            <h3 style="font-size: 1.5rem; margin: 0; color: white; font-weight: 700;">{{ $stats['total_borrows'] }}</h3>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 1.25rem; margin-bottom: 1.5rem;">
    <div class="chart-container">
        <h3 class="chart-title">🔔 Đặt Sách Chờ Xử Lý ({{ $pendingReservations->count() }})</h3>
        @if ($pendingReservations->count() > 0)
            <div style="display:flex; flex-direction:column; gap:0.6rem; max-height:300px; overflow:auto;">
                @foreach ($pendingReservations as $reservation)
                    <div style="padding:0.7rem 0.8rem; border-radius:8px; background:#f0f9ff; border-left:3px solid #0284c7;">
                        <div style="font-weight:600; color:#0f172a;">{{ $reservation->book->title ?? 'N/A' }}</div>
                        <div style="font-size:0.85rem; color:#475569; margin-top:0.25rem;">Thành viên: {{ $reservation->user->name ?? 'N/A' }}</div>
                        <div style="font-size:0.8rem; color:#64748b; margin-top:0.2rem;">Đặt lúc: {{ optional($reservation->reserved_at)->format('d/m/Y H:i') }}</div>
            ...
            <div style="margin-top:0.65rem; text-align:right;">
                <a href="{{ route('borrows.reservations') }}" style="color:#0284c7; text-decoration:none; font-size:0.88rem; font-weight:600;">Xem toàn bộ yêu cầu mượn →</a>
            </div>
        @else
    ...
    <div class="chart-container">
        <h3 class="chart-title">💳 Yêu Cầu Nạp Ví ({{ $pendingTopupRequests->count() }})</h3>
        @if ($pendingTopupRequests->count() > 0)
            <div style="display:flex; flex-direction:column; gap:0.7rem; max-height:300px; overflow:auto;">
                @foreach ($pendingTopupRequests as $requestLog)
                    <div style="padding:0.8rem; border-radius:8px; background:#f8fafc; border-left:3px solid #0ea5e9;">
                        <div style="display:flex; justify-content:space-between; gap:0.5rem; align-items:flex-start;">
                            <div>
                                <div style="font-weight:600; color:#0f172a;">{{ $requestLog->causer->name ?? 'Thanh vien' }}</div>
                                <div style="font-size:0.85rem; color:#475569; margin-top:0.2rem;">Số tiền: {{ number_format((int) data_get($requestLog->properties, 'amount', 0)) }} VND</div>
                                @if (data_get($requestLog->properties, 'note'))
                                    <div style="font-size:0.8rem; color:#64748b; margin-top:0.2rem;">Ghi chú: {{ data_get($requestLog->properties, 'note') }}</div>
                                @endif
                            </div>
                            <form action="{{ route('wallet-topups.approve', $requestLog->id) }}" method="POST" style="display:inline;">
                                @csrf
                    ...
                    <tbody>
                        @foreach ($popular_books as $book)
                            <tr style="font-size: 0.9rem;">
                                <td style="padding: 0.6rem 1rem;"><strong>{{ Str::limit($book->title, 25) }}</strong></td>
                                <td style="padding: 0.6rem 1rem; font-size: 0.85rem;">{{ Str::limit($book->author, 18) }}</td>
                                <td style="padding: 0.6rem 1rem; text-align: center; color: #f59e0b; font-weight: 600;">⭐ {{ $book->rating }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 0.5rem; max-height: 320px; overflow-y: auto;">
                @foreach ($categories as $category)
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.6rem 0.8rem; background: linear-gradient(135deg, rgba(6, 182, 212, 0.05) 0%, rgba(8, 145, 178, 0.03) 100%); border-radius: 8px; border-left: 3px solid #06b6d4; font-size: 0.9rem;">
                        <span style="font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin-right: 0.5rem;" title="{{ $category->name }}">{{ Str::limit($category->name, 20) }}</span>
                        <span style="background: linear-gradient(135deg, #06b6d4 0%, #0891b2 100%); color: white; padding: 0.2rem 0.6rem; border-radius: 16px; font-size: 0.8rem; font-weight: 700; flex-shrink: 0;">{{ $category->books_count }}</span>
                    </div>
                @endforeach
            </div>
        @else
            ...
            <table class="table" style="margin-bottom: 0; font-size: 0.9rem;">
                ...
                <tbody>
                    @foreach ($recent_borrows as $borrow)
                        <tr style="font-size: 0.9rem;">
                            <td style="padding: 0.6rem 1rem;">{{ Str::limit($borrow->user->name, 20) }}</td>
                            <td style="padding: 0.6rem 1rem;">{{ Str::limit($borrow->book->title, 25) }}</td>
                            <td style="padding: 0.6rem 1rem; font-size: 0.85rem;">{{ $borrow->borrowed_at->format('d/m/Y') }}</td>
...
<script>
    ...
    const borrowsByMonth = @json($borrowsByMonth);
