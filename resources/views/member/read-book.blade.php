...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; gap: 0.75rem; flex-wrap: wrap;">
    <div>
        <h2 style="margin: 0;">📖 Đọc Sách Số {!! $isVIP ? '<span class="badge bg-warning" style="color:#000; background:linear-gradient(135deg, #fcd34d, #f59e0b);">TÀI KHOẢN VIP</span>' : '' !!}</h2>
        <p style="margin: 0.35rem 0 0; color: #6b7280;">{{ $borrow->book->title }} - {{ $borrow->book->author }}</p>
    </div>
...
<div style="background: #fff7ed; border-left: 4px solid #f97316; color: #9a3412; padding: 0.85rem 1rem; border-radius: 8px; margin-bottom: 1rem; display: flex; align-items: center; justify-content: space-between;">
    ...
    <a href="{{ route('dashboard') }}" class="btn btn-warning btn-sm" style="white-space: nowrap;"><i class="fas fa-gem"></i> Nạp Ví Lên 1 Triệu</a>
</div>
@endif
...
<div class="reader-container" id="reader-container">
    ...
    <div class="reader-toolbar">
        ...
        <div class="toolbar-group">
            <button class="reader-btn {{ !$isVIP ? 'vip-lock' : '' }}" id="btn-rotate" title="Xoay dọc/ngang">
                <i class="fas fa-sync-alt"></i><span class="d-none d-md-inline"> Xoay</span>
                @if(!$isVIP)<i class="fas fa-lock"></i>@endif
            </button>
            <button class="reader-btn {{ !$isVIP ? 'vip-lock' : '' }}" id="btn-bookmark" title="Đánh dấu trang hiện tại">
                <i class="fas fa-bookmark"></i>
                @if(!$isVIP)<i class="fas fa-lock"></i>@endif
            </button>
            <button class="reader-btn {{ !$isVIP ? 'vip-lock' : '' }}" id="toggle-sidebar" title="Danh sách trang đánh dấu">
                <i class="fas fa-list-ul"></i>
                @if(!$isVIP)<i class="fas fa-lock"></i>@endif
            </button>
            <button class="reader-btn {{ !$isVIP ? 'vip-lock' : '' }}" id="toggle-dark" title="Chế độ ban đêm">
    ...
    <div class="canvas-wrapper" id="canvas-wrapper">
        ...
        <div class="watermark-overlay">
            <div class="watermark-text">
                @for ($i = 0; $i < 30; $i++)
                    <span>{{ auth()->user()->email }} | TÀI LIỆU MẬT</span>
                @endfor
            </div>
        </div>
    </div>
...
<script>
    ...
    const url = '{{ route('member.read-book.stream', ['borrowId' => $borrow->id]) }}';
